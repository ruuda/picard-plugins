...
PLUGIN_NAME = 'Release Language Title'
PLUGIN_AUTHOR = 'Bob Swift (rdswift)'
PLUGIN_DESCRIPTION = '''
This plugin provides full language title for the release as a
variable (_releaselanguagetitle) for use in naming scripts.
'''
...
def make_releaselanguagetitle(album, album_metadata, release_metadata):
    album_id = release_metadata['id']
    if album_metadata['~releaselanguage']:
        lang = album_metadata['~releaselanguage']
    else:
        log.error("{0}: {1}: Missing '~releaselanguage' in metadata.".format(
                PLUGIN_NAME, album_id))
        lang = 'und'
    if LANGUAGE[lang]:
        album_metadata['~releaselanguagetitle'] = LANGUAGE[lang]
    else:
        log.error("{0}: {1}: Unknown '~releaselanguage' in metadata: '{2}'".format(
                PLUGIN_NAME, album_id, lang))
        album_metadata['~releaselanguagetitle'] = 'Unknown'


register_album_metadata_processor(make_releaselanguagetitle)
